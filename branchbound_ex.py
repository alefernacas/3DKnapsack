...
class Box:
    def __init__(self, width, height, depth, value, weight):
...
# Función para optimizar la colocación de cajas en el contenedor
def optimize_container(container, boxes):
    best_value = 0 #Valor de container mas alto obtenido
    best_placement = [] #Combinancion de cajas correspondiente a ese valor
    ...
    # Probar todas las combinaciones de cajas y orientaciones
    for num_boxes in range(1, len(boxes) + 1):
        #La función combinations genera combinaciones de num_boxes cajas seleccionadas del conjunto de todas las cajas
        for box_combination in combinations(boxes, num_boxes):
            #Se prueban todas las posibles orientaciones de esas cajas usando product
            for orientations in product(*(box.orientations for box in box_combination)):
                #Eliminamos cualquier caja de pruebas colocada
                container.placed_boxes.clear()
                current_value = 0

                # Tratar de colocar las cajas en esta combinación
                if place_boxes_in_container(container, box_combination, orientations):
                    current_value = container.calculate_total_value()

                # Guardar la mejor colocación
                if current_value > best_value:
                    best_value = current_value
                    best_placement = container.placed_boxes.copy()

    # Restaurar la mejor colocación en el contenedor
    container.placed_boxes = best_placement
    return best_value

# Función para intentar colocar una lista de cajas con una lista de orientaciones en el contenedor
def place_boxes_in_container(container, boxes, orientations):
    for box, orientation in zip(boxes, orientations): #Emparejamos cada caja con su orientacion
        placed = False #Establecemos si la caja actual ha sido colocada o no en el contenedor
        # Probar todas las posiciones posibles en el contenedor (primero desde la base, es decir, z=0)
        for z in range(container.height): #Comenzamos el bucle desde la base
            for y in range(container.depth): #Colocamos desde el frente hacia el fondo -> Deberiamos invertirlo
                for x in range(container.width): #Coloca de izq a der
                    #Llamamos a las funciones para comprobar si podemos colocar la caja 
                    if container.can_place_box(box, x, y, z, orientation):
                        container.place_box(box, x, y, z, orientation)
                        placed = True #La caja ha sido colocada e interrumpimos los bucles
                        break
                if placed:
                    break
            if placed:
                break
        if not placed:
            return False  # Si alguna caja no se puede colocar, fallar -> Deberiamos cambiarlo(añadir cajas no colocadas a otra lista???)
    return True #Si todas se han colocado devolvemos el mensaje trues

# Ejemplo de uso
boxes = [
    # Cajas de peso pesado (peso > 15)
    Box(3, 4, 2, 100, 20),  # Caja 1: Peso 20, dimensiones (3x4x2)
    Box(5, 2, 3, 90, 18),   # Caja 2: Peso 18, dimensiones (5x2x3)
    Box(4, 4, 4, 120, 22),  # Caja 3: Peso 22, dimensiones (4x4x4)
    Box(3, 3, 6, 110, 21),  # Caja 4: Peso 21, dimensiones (3x3x6)
    Box(2, 5, 3, 95, 19),   # Caja 5: Peso 19, dimensiones (2x5x3)
    Box(3, 6, 3, 115, 23),  # Caja 6: Peso 23, dimensiones (3x6x3)

    # Cajas de peso medio (peso entre 10 y 15)
    Box(2, 3, 4, 60, 12),   # Caja 7: Peso 12, dimensiones (2x3x4)
    Box(3, 4, 2, 65, 14),   # Caja 8: Peso 14, dimensiones (3x4x2)
    Box(5, 3, 2, 75, 15),   # Caja 9: Peso 15, dimensiones (5x3x2)
    Box(4, 2, 3, 70, 13),   # Caja 10: Peso 13, dimensiones (4x2x3)
    Box(2, 2, 4, 55, 11),   # Caja 11: Peso 11, dimensiones (2x2x4)
    Box(3, 3, 3, 85, 14),   # Caja 12: Peso 14, dimensiones (3x3x3)
    Box(2, 4, 3, 80, 13),   # Caja 13: Peso 13, dimensiones (2x4x3)
    Box(4, 3, 2, 77, 14),   # Caja 14: Peso 14, dimensiones (4x3x2)
    Box(3, 5, 2, 82, 15),   # Caja 15: Peso 15, dimensiones (3x5x2)

    # Cajas de peso ligero (peso < 10)
    Box(2, 3, 2, 40, 8),    # Caja 16: Peso 8, dimensiones (2x3x2)
    Box(1, 3, 3, 45, 9),    # Caja 17: Peso 9, dimensiones (1x3x3)
    Box(2, 2, 2, 35, 7),    # Caja 18: Peso 7, dimensiones (2x2x2)
    Box(3, 2, 1, 30, 6),    # Caja 19: Peso 6, dimensiones (3x2x1)
    Box(1, 2, 3, 25, 5),    # Caja 20: Peso 5, dimensiones (1x2x3)
    Box(2, 1, 1, 20, 4),    # Caja 21: Peso 4, dimensiones (2x1x1)
    Box(2, 1, 2, 28, 7),    # Caja 22: Peso 7, dimensiones (2x1x2)
    Box(1, 3, 2, 32, 8),    # Caja 23: Peso 8, dimensiones (1x3x2)
    Box(2, 2, 1, 27, 6),    # Caja 24: Peso 6, dimensiones (2x2x1)
    Box(1, 1, 2, 22, 5),    # Caja 25: Peso 5, dimensiones (1x1x2)
    Box(2, 1, 3, 35, 9),    # Caja 26: Peso 9, dimensiones (2x1x3)
    Box(1, 2, 2, 33, 8),    # Caja 27: Peso 8, dimensiones (1x2x2)
    Box(2, 1, 1, 26, 5),    # Caja 28: Peso 5, dimensiones (2x1x1)
    Box(1, 1, 1, 20, 4),    # Caja 29: Peso 4, dimensiones (1x1x1)
    Box(3, 2, 2, 38, 9)     # Caja 30: Peso 9, dimensiones (3x2x2)
]

container = Container(5, 5, 5)
